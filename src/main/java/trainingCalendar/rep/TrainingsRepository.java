...
public interface TrainingsRepository extends JpaRepository<Training, Long> {
    @Query(value = "SELECT * FROM trainings t WHERE t.actual = true AND EXTRACT(MONTH FROM t.date) = :month AND EXTRACT(YEAR FROM t.date) = :year AND t.city IN (:cities)", nativeQuery = true)
